...
def get_model_args(params):
    """ Function to organize the inputs from the Parametrization class. this dictionary are 
    matchs the keyword arguments of the TubularBasePlateModel class that constructu the geometry of the baseplate"""
    return  {
...
def generate_model(kwargs) -> tuple[dict[str, dict], dict[str, dict], list[str]]:
    """Function to create the topology and mesh of the baseplate. It uses the output of get_model_args and then construct the baseplate model """
    model = TubularBasePlateModel(**kwargs)
    nodes, triangles, supports = model.build()
    # Get the boundary nodes of the anchor bolts to apply constraints
    hole_boundaries = model.get_hole_boundary_node_tags()
    # Get the top nodes of the cylinder 
...
def run_staad_analysis(kwargs, plate_thickness:float, loads: dict|None = None):
    """ Function to create and run the model in STAAD.PRO using the VIKTOR Woker
    along a PythonAnalysis"""
    # Generate geometry
    ...
    if loads:
        ...
        loads = []
    # Serialize input for the worker
    ...
    # Perform the analysis with the viktor worker
...
class Controller(vkt.Controller):
    ...
    def render_model(self, params, **kwargs)-> vkt.GeometryResult:
        """ Function to render the baseplate 3D model/mesh with loads"""
        # Get model args from app
        ...
        # Render the 3D model structural or mesh based on params.step_1.mode
        objects = render_model(nodes=nodes,triangles=triangles,mesh_mode=params.step_1.mode)
        # Render bolts and foundation block if params.step_1.mode is Fale
        ...
        # Render loads if params.step_2.loads is not empty
    ...
    def results(self, params, **kwargs)->vkt.GeometryResult:
        ...
        if loads:
            ...
            load_list = []
        # Run the analysis and retrieve the von missess stress and displacements
